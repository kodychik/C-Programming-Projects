...
User *create_user(char *name)
{
  ...
  if (in_friend_list(allUsers, new_user)) return NULL;
...
int delete_user(User *user)
{
  ...
  if (!in_friend_list(allUsers, user) || user == NULL) return -1;
...
{
  if (user == NULL || friend == NULL || !in_friend_list(allUsers, user) || !in_friend_list(allUsers, friend)) return -1;
  if (in_friend_list(user->friends, friend) || in_friend_list(friend->friends, user)) return -1;
  if (strcmp(user->name, friend->name) == 0) return -1;
...
{
  if (user == NULL || !in_friend_list(allUsers, user)) return -1;
  if (get_brand_index(brand_name) < 0 || in_brand_list(user->brands, brand_name)) return -1;
...
int get_mutual_friends(User *a, User *b)
{
  ...
  if (a == NULL || b == NULL || !in_friend_list(allUsers, a) || !in_friend_list(allUsers, b)) return 0;
  int count = 0;
  FriendNode *p = NULL;
  User *q = NULL;
  p = b->friends;
  ...
  {
    q = p->user;
    if (in_friend_list(a->friends, q)) count++;
    p = p->next;
  }
...
{
  if (in_friend_list(a->friends, b)) return a->depth;
  if (a->depth >= lengthOfAllUsers() - 1) return -1;
  ...
  if (!isEmpty(head)) head = deletingTheYard(head);
...
void printingUsers()
{
  ...
  {
    printf("%s\n", p->user->name);
    if (p->user->visited) printf("User has been visited\n");
    else printf("User has not been visited\n");
    p = p->next;
  }
}
...
{
  // return shortest number of steps to get from user a to b
  /*
  b->depth++;
  FriendNode *p = NULL;
  p = a->friends;
  
  //if (get_mutual_friends(a, b) > 0) return depth + 2;

  if (strcmp(a->name, b->name) == 0) return 0; // if depth larger than certain value should return -1
  // user a -> friends, insert each friend of a until FN->next is null, insert all these into queue
  // then use queue, check for each item in queue using in_friend_list helper function -> if true then returh b->depth, else none of 
  
  if (in_friend_list(p, b)) return b->depth;
...
void connect_similar_brands(char *brandNameA, char *brandNameB)
{
  ...
  (get_brand_index(brandNameA) < 0 || get_brand_index(brandNameB) < 0)) return;
...
User *get_suggested_friend(User *user)
{
  ...
  while (p != NULL)
  {
    if (strcmp(user->name, p->user->name) != 0 && !in_friend_list(user->friends, p->user)) //checks if user is itself, user is already friends with user and if user has been visited already
    {
      ...
      while (ptr != NULL)
      {
        if (in_brand_list(user->brands, ptr->brand_name)) count++;
        ptr = ptr->next;
      }
      if (count >= largest)
      {
  ...
  for (k = 0; k < count1; k++)
  {
    for (i = 0; i < MAT_SIZE; i++)
    {
      if (!in_brand_list(user->brands, brand_names[i]) && !ifInArray(brandNames, get_brand_index(brand_names[i]), count1))
      {
        ...
        {
          if (brand_adjacency_matrix[i][j] == 1 && in_brand_list(user->brands, brand_names[j])) count++; // checking similar in matrix
        }
        if (count > largest)
        {
